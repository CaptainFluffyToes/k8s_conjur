...
standby_config()
{
	...
	kubectl -n $NAMESPACE exec $master_pod_name -c $master_container_name -- evoke ca issue --force $STANDBY_URL
	...
	for pod in $standby_pod_names; do
		printf "\nWorking on Pod:$pod.\n"
#		kubectl cp $NAMESPACE/$pod:/standby.tar -c $standby_container_name standby.tar 
...
authnk8s_config()
{
	...
	local authnk8s_container_name=follower
	local master_pod_name=$(kubectl get pods -n $NAMESPACE -l app=conjur-node,role=master --no-headers | awk '{print $1;}')
	local master_container_name=master
	...
	for pod in $authnk8s_pod_names; do
		printf "\nWorking on Pod:$pod\n"
		kubectl cp authnk8s.tar $NAMESPACE/$pod:/authnk8s.tar -c $authnk8s_container_name
		printf "\n\nUnpacking and importing authnk8s seed package.\n\n"
		kubectl -n $NAMESPACE exec $pod -c $authnk8s_container_name evoke unpack seed /authnk8s.tar
		kubectl -n $NAMESPACE exec $pod -c $authnk8s_container_name evoke configure follower
	done
